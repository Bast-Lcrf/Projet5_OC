...
{
    // Récupère les commentaires associés à son article
    ...
    // ajout d'un commentaire avec restriction de validation
    ...
    // Supprime les commentaires d'un article lors de sa suppression
        public function deleteCommentFromArticle($idArticle) 
        {
            $db = $this->dbConnect();
...
{
    // Récupère les commentaires en attente de validation par la modération
    ...
    // Mets à jour les commentaires validé après modération
        public function updateCom($idComment) 
        {
            $db = $this->dbConnect();
    ...
    // supression du commentaire par la modération
        public function deleteCom($idComDelete) 
        {
            $db = $this->dbConnect();
            $delete = $db->prepare('DELETE FROM Comments WHERE id_comment = ?');
            $delete->execute(array($idComDelete));
        }

    // Commentaire modifier par l'utilisateur --> retour validation
        public function modifyCom($com, $idCom) 
        {
            $db = $this->dbConnect();
            $modifyCom = $db->prepare('UPDATE Comments SET comment = ?, validation_com = 2, date_comment = NOW() WHERE id_comment = ?');
            $modifyCom->execute(array($com, $idCom));
        }
}